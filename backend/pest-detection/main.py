...
# Load models
dense_model = tf.keras.models.load_model("models/papaya_disease_model.h5")
eff_model = tf.keras.models.load_model("models/efficient_classify_model.h5")
...
def predict_dense(img_array):
    ...
    dense_index = int(np.argmax(predictions[0])) #get highest probability
...
if __name__ == "__main__":
    uvicorn.run(app,  host="0.0.0.0", port=5014) #host = 0.0.0.0 when connect with android emulator
